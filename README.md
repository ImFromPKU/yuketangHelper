...
- 更新时间：2021 年 5 月底
- 修改为默认适用于清华大学学生（pro.yuketang.cn）
- 将原版使用的已失效 API 更换为新的 API
- 调整模拟观看速度至与实际观看速度近似，并更精细地模拟心跳数据包，以规避风险
- 调整代码结构，便于维护
- 独立用户自定义配置，不再需要修改 Python 文件本身

### 配置说明

请将 `config_example.json` 复制一份，改名为 `config.json` 以使用。

- `csrftoken` 和 `sessionid`：登录雨课堂，按 F12 -> 选 Application -> 找到雨课堂的 cookies，将其中这两个字段的内容填入配置。
- `university-id`：大学对应的 id，默认为 2598，对应清华大学。可以按照下方原作者的说明找到。欢迎在 issues 中提供其他学校的 id。
- `ykt_address`：雨课堂域名，根据大学不同。
- `heartrate` 和 `heartbeat_pack_size`：前者表示多少秒生成一次心跳数据，后者表示生成多少个心跳数据后将它们上传到服务器。默认值与雨课堂网页版的行为相同，即一倍速正常观看。修改这两个值也许可以调整模拟观看速度，但我未进行过测试，后果自负。
